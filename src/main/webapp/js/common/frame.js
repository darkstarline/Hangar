...
$(document).ready(function(){
	...
	$.each($(".nav li a"), function(){
		...
		if(actionPath == href || actionPath == (contextPath +"/" + href)){
...
(function($){
	...
	*/
	$.message = function(text, options){
		var opts = {title:"系统提示", icon: "succeed", content: text, lock: true, time: 2};
		if(null != opts){
	...
	$.error = function(text, options){
		var opts = {title:"系统提示", icon: "error", content: text, lock: true, time: 2};
		if(null != opts){
			opts = $.extend(opts, options);
		}
		art.dialog(opts);
	...
	*/
	$.fn.goAjax = function(parameters){
		...
		var bCheckLogin = null==parameters.checkLogin?false:parameters.checkLogin;
...
function ajaxhtml(obj,href,fn){
	...
	var url = href + (0 < href.indexOf("?")?"&t=":"?t=") + dt; 
	$.getEx(url, {}, function(data){
		$(obj).html(data);
		if(fn){fn();}
	}, "html");
}
		...
		success: function(response) {
			if(false == response.result){
				art.dialog.open('user/loginPanel', {
					...
					}, lock: true, id: "loginPanelDialog", top: '15%'
				}, false);
			} else {
				if(fn){fn();}
			}
...
$.getEx = function(url, getdata, callback, dataType) {
    $.ajax({
        ...
        url: url,
        data: getdata,
        dataType: null == dataType?"jsonp":dataType,//解决跨域（跨域访问就是你在一个域环境下，访问另一个域的内容），用户传递一个callback参数给服务端，然后服务端返回数据时将这个callback参数作为行数么来包裹json数据，客户端就可以随意定制自己的函数来返回数据了；
		jsonp:"callback",
		cache: true,
        ...
        error: function() {
            callback(eval('({error_msg:"网络错误！请您稍后重试此操作。"})'));
        }
    });
};

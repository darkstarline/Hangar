    ...
    Page.operate = {
        ...
        tt: function() {
            $(".btn").on('click', function(event) {
                var cmd = Page.action.getParams($form);
                cmd = this.jsonToStr(cmd);
                Ajax.postJson(API.get(''), cmd, function (json, state) {
                    if (state) {
                        $.message('修改成功!');
                        $addAuth.close(); // 关闭修改弹框
                        $table.refresh();//刷新表格
                    } else {
                        $.message({message: json.message, type: 'warning'});
                    }
                })
            });
        },
